---
title: "Fontes"
description: "Bibliografia completa das fontes utilizadas, agrupadas por capítulo"
weight: 30
---

# Fontes

*Bibliografia completa de todas as fontes utilizadas neste livro, agrupadas por capítulo.*

> A consolidação num índice único está em preparação. Até lá, cada capítulo lista as suas fontes nas notas de rodapé no final do capítulo, e a edição-mãe russa serve de referência estrutural.

## Estrutura

A secção de fontes organiza-se segundo cinco categorias, seguindo a edição-mãe russa como referência estrutural.

### 1. Bibliografia geral

Fontes referenciadas em vários capítulos: clássicos da gestão de projetos (Deming, Ohno, Womack), documentação corporativa histórica (Toyota, Boeing 737 MAX, Microsoft), enquadramentos teóricos de decisão sob incerteza (Snowden e Boone «Cynefin», Kahneman «Sistema 1 e Sistema 2»), estudos comparados de transformação organizacional (McKinsey, MIT IMVP).

### 2. Fontes por capítulo

Fontes específicas de cada capítulo, com números de página quando aplicável. Estão listadas nas notas de rodapé no final de cada um dos doze capítulos e do posfácio. Este ficheiro consolidará essas notas num índice único; até lá, valem as notas de rodapé de cada capítulo.

### 3. Textos canónicos em português europeu

Quando existe edição consolidada em PT-EU de uma obra literária citada — Verne, Shelley, Conan Doyle, Cervantes, Camus, Stevenson, Wells, entre outros — é essa a referência primária. Sempre que possível, recorre-se a traduções reconhecidas de editoras portuguesas (Livros do Brasil, Publicações Europa-América, Relógio d'Água, Teorema, Círculo de Leitores, Presença, Guerra & Paz, Dom Quixote, entre outras). A edição concreta usada em cada citação encontra-se na nota de rodapé correspondente.

### 4. Textos em línguas originais

Quando não existe edição PT-EU canónica de uma obra citada — sobretudo textos académicos e corporativos em inglês (livros de gestão, artigos científicos, relatórios governamentais), documentação técnica em francês, alemão ou italiano — a citação faz-se do texto original. A primeira ocorrência de cada obra dentro de um capítulo é marcada com **«(tradução da redação)»**.

### 5. Fontes russas dos irmãos Strugatski

No capítulo 9, a obra *Понедельник начинается в субботу* («Segunda-feira Começa ao Sábado», 1965) é citada a partir do texto original russo. Não existindo edição PT-EU canónica desta obra, todas as citações levam **«(tradução da redação)»** marcado explicitamente na primeira ocorrência dentro do capítulo.

## Convenção «tradução da redação»

Sempre que uma citação provém de obra sem edição PT-EU consolidada — russo original dos Strugatski, artigos académicos em inglês, relatórios governamentais em francês, documentação técnica em alemão, entre outros — a primeira ocorrência dentro de um capítulo é marcada «(tradução da redação)». As ocorrências seguintes dentro do mesmo capítulo dispensam a repetição do marcador. A norma segue a prática das edições académicas portuguesas.

## Fontes canónicas de referência

Até este ficheiro consolidar a totalidade das notas num índice único, as referências canónicas estão disponíveis em:

- Notas de rodapé no fim de cada capítulo (todos os doze capítulos e o posfácio): [Capítulos →](/kn1/pt/read/)
- Fontes da edição-mãe russa: [Fontes em russo →](/kn1/pt/read/apparatus-sources) (referência estrutural principal)

---

*[Ao Índice ↗](/kn1/pt/read)*
