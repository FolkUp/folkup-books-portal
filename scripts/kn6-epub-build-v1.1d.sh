#!/bin/bash
#
# kn.6 «Своими глазами» EPUB v1.1d generator (pandoc-based)
# S1KOCHEGAR cont+13 · 2026-09-06 (per Iskra KANON EPUB-TRAKT-1 S315-04)
#
# CHANGES vs v1.1c (KANON EPUB-TRAKT-1):
#   - Обложка = K1 «Картотека» Фриды (vault/books/kn6-package/covers/kn6-cover-final.png)
#     вместо типографского титула (public/covers/cover_kn6.png)
#   - Nav.xhtml linear="no" в spine (постпроцесс OPF после pandoc build)
#
# CHANGES retained from v1.1c:
#   - RAW HTML img без figure/figcaption (no technical captions in body)
#   - webp → jpg conversion (universal reader support)
#   - No --split-level explicit flag (matches v1.0)
...
BUILD_DIR="/c/Transit/kn6-build-v1.1d"

EPUB_VERSION="${1:-v1.1d}"
...
# Step 5 — POSTPROCESS: nav.xhtml linear="no" в spine per КАНОН S315-04 §2.1
echo ""
echo "🔧 Postprocess: nav linear=\"no\" in spine (per КАНОН EPUB-TRAKT-1 §2.1)..."

python3 - <<'PYEOF'
import zipfile, shutil, os, sys, re

OUTPUT = os.environ.get('OUTPUT_EPUB', '/c/Transit/kn6-build-v1.1d/kn6-svoimi-glazami-v1.1d.epub')
    ...
    with zipfile.ZipFile(TEMP, 'w', zipfile.ZIP_DEFLATED) as z_out:
        for item in z_in.infolist():
            ...
            if item.filename == 'EPUB/content.opf':
                ...
                # Replace nav itemref: `<itemref idref="nav" />` → `<itemref idref="nav" linear="no" />`
                old_nav = '<itemref idref="nav" />'
                new_nav = '<itemref idref="nav" linear="no" />'
                if old_nav in content:
                    content = content.replace(old_nav, new_nav)
                    print(f'  [OK] Nav itemref linear="no" applied')
                else:
                    print(f'  [WARN] Nav itemref pattern not found, checking variations...')
...
# Verify postprocess
echo ""
echo "🔍 Verify nav linear=\"no\":"
unzip -p "$OUTPUT_EPUB" EPUB/content.opf | grep -oE '<itemref idref="nav"[^/]*/>'
...
echo ""
echo "🎉 kn.6 EPUB v1.1d build complete!"
