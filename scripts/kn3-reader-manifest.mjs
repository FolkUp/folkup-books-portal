 ...
 * Output:
 *   - content/kn3/ru/chapters-generated/{slug}.md    — one per chapter section (legacy, kept для tooling)
 *   - content/kn3/ru/chapters-manifest.json          — metadata index
 *   - content/kn3/ru/chapters-html/{slug}.js         — pre-rendered HTML per chapter, ES modules
 ...
 * VIT-KLB cont+51 mirror: HTML pre-render at build time added (mirrors kn1 PR #146 pattern) —
 *   fixes async-watcher SSR non-suspend, ensures GoogleBot sees real content не placeholder.
 */
...
const BODIES_DIR = resolve(PROJECT_ROOT, 'content/kn3/ru/chapters-html')
 ...
 * precommit:allow-ai-mentions magic marker — index contains legit names like «Monet, Claude»
 * which false-positive the AI-name scanner (Kropotkin, Kropotkin, Kaczmarek, etc. clear).
 */
function serializeHtmlModule(html) {
  return `// Auto-generated by scripts/kn3-reader-manifest.mjs — DO NOT EDIT.
...
function main() {
  ...
  // Clean + recreate chapters-generated/ + chapters-html/
  ...
  // Write per-chapter markdown files (only reader-included) + per-chapter HTML modules
  let totalHtmlSize = 0
  let maxHtmlSize = 0
  let maxHtmlSlug = ''
  ...
  console.log(
    `[kn3-reader-manifest] Wrote ${entries.length} HTML modules к ${BODIES_DIR}`,
  )
  console.log(
    `[kn3-reader-manifest]   total: ${(totalHtmlSize / 1024).toFixed(1)} KB, max: ${(maxHtmlSize / 1024).toFixed(1)} KB (${maxHtmlSlug})`,
  )
}
