#!/bin/bash
#
# kn.1 «Agile Sapiens» (DE edition) EPUB v1.0.0 generator (pandoc-based)
...
#   §2.2 Cover = Frida art `cover_kn1_de.v3.png` (не типографский титул), resize 1200x1800
...
#
# Content sources: folkup-books-portal/content/kn1/de/
...
#   - Rights CC BY-SA 4.0
#   - Language de, translator Bolik
#
# Prerequisites: pandoc 3.x, ImageMagick (magick), python3, epubcheck 5.x
#
# Usage: bash scripts/kn1-de-epub-build.sh [VERSION]
...
CONTENT_ROOT="$PROJECT_ROOT/content/kn1/de"
...
COVER_PNG="$PROJECT_ROOT/public/covers/cover_kn1_de.v3.png"

BUILD_DIR="/c/Transit/kn1-de-build"
EPUB_VERSION="${1:-v1.0.0}"
OUTPUT_EPUB="$BUILD_DIR/agile-sapiens-de-${EPUB_VERSION}.epub"

echo "📚 kn.1 «Agile Sapiens» (PT) EPUB build ${EPUB_VERSION} (КАНОН EPUB-TRAKT-1)"
echo "==========================================================================="
echo "  Cover: cover_kn1_de.v3.png (Frida v3 art 31.08) → resize 1200x1800"
...
MERGED_MD="$BUILD_DIR/kn1-de-merged.md"

KN1_DE_CONTENT_ROOT="$CONTENT_ROOT" python3 - <<'PYEOF'
import json, os, re, sys

CONTENT_ROOT = os.environ.get("KN1_DE_CONTENT_ROOT", "/c/JOHNDOE_CLAUDE/folkup-books-portal/content/kn1/de")
BUILD_DIR = "/c/Transit/kn1-de-build"
...
merged_path = native(f"{BUILD_DIR}/kn1-de-merged.md")
...
def strip_footer_link(text):
    ...
    # Pattern: '---\n\n*[Ao Índice ↗](/kn1/de/read)*' at file end
...
def strip_reader_urls(text):
    """Replace absolute /kn1/de/read/* cross-reference links with plain text (canon §RSC-026 fix)

    Web reader has these URLs; EPUB cannot resolve them (RSC-026 URL leaks outside container).
    Pattern: [text](/kn1/de/read/slug) → text (keeping the display text, dropping the link)
    """
    text = re.sub(r'\[([^\]]+)\]\(/kn1/de/read[^\)]*\)', r'\1', text)
...
# Step 4 — POSTPROCESS Template v4: MOVE nav.xhtml itemref в spine ПОСЛЕ введения (Preface)
# Iskra POMETKA-S317-06 mandate: «поправить Template v4 все три EPUB kn6 v1.1g kn1 PT v1.0.1 kn1 DE v1.0»
# S315-06 REMOVE-nav-from-spine pattern superseded — читалки не показывали ncx TOC page.
# Template v4 restores nav в spine as visible chapter after preface — reader shows оглавление как page.
echo ""
echo "🔧 Postprocess Template v4: MOVE nav itemref после введения в spine (per Iskra POMETKA-S317-06 + POMETKA-S318-05 §2 п.2 mandate all 3 EPUB)..."
    ...
    with zipfile.ZipFile(TEMP, 'w', zipfile.ZIP_DEFLATED) as z_out:
        for item in z_in.infolist():
            ...
            if item.filename == 'EPUB/content.opf':
                ...
                # Template v4: MOVE nav itemref ПОСЛЕ первого content itemref (preface = Vorwort)
                # Sequence: cover_xhtml → title_page_xhtml → ch001_xhtml (Vorwort) → NAV → ch002_xhtml ...
                ...
                if nav_match:
                    ...
                    # Find first CHAPTER itemref (skip cover/title/nav, find ch001 = Vorwort)
                    ...
                    if intro_match:
                        ...
                        nav_moved = 1
                        print(f'  [OK] Template v4: nav itemref MOVED после ch001 (Vorwort) per Iskra POMETKA-S318-05 §2 п.2')
                    else:
                        ...
                        if first_chapter:
                            ...
                            if match:
                                ...
                                nav_moved = 1
                                print(f'  [OK] Template v4: nav itemref MOVED после первой главы (idref={first_chapter}) — fallback')
                            else:
                                print(f'  [WARN] Template v4: fallback failed, nav removed from spine')
                                content = content_no_nav
                        else:
                            print(f'  [WARN] Template v4: no chapter itemref found, nav removed from spine')
                            content = content_no_nav
                else:
                    print(f'  [INFO] Template v4: no nav itemref in spine to move')
                data = content.encode('utf-8')
            # v1.0 Template v4: KEEP landmarks TOC reference since nav IS в spine (no removal needed)
...
print(f'  [OK] Template v4 nav moves: {nav_moved}')
...
# Gate 1 Template v4: nav MUST be present в spine (position >1) per Iskra POMETKA-S318-05 §2 п.2
NAV_IN_SPINE=$(unzip -p "$OUTPUT_EPUB" EPUB/content.opf 2>/dev/null | grep -c 'idref="nav"' || echo "0")
echo "  Gate 1 (nav в spine Template v4): $NAV_IN_SPINE (must be 1)"
...
# Gate 9: Bolik translator credit
ZEKA_COUNT=$(unzip -p "$OUTPUT_EPUB" 'EPUB/text/*.xhtml' 2>/dev/null | grep -cE '\bBolik\b' || echo "0")
echo "  Gate 9 (Bolik translator ≥ 1): $ZEKA_COUNT"
