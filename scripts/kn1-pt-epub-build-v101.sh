#!/bin/bash
#
# kn.1 «Agile Sapiens» (PT edition) EPUB v1.0.1 generator (pandoc-based)
...
BUILD_DIR="/c/Transit/kn1-pt-build-v101"
...
# Step 4 — POSTPROCESS Template v4: MOVE nav.xhtml itemref в spine ПОСЛЕ введения (Prefácio)
# Iskra POMETKA-S317-06 mandate «all 3 EPUB kn6 v1.1g kn1 PT v1.0.1 kn1 DE v1.0» + POMETKA-S318-05 §2 п.2/3
...
echo ""
echo "🔧 Postprocess Template v4: MOVE nav itemref после введения (Prefácio) в spine (per Iskra POMETKA-S318-05 §2 п.3 mandate all 3 EPUB)..."
    ...
    with zipfile.ZipFile(TEMP, 'w', zipfile.ZIP_DEFLATED) as z_out:
        for item in z_in.infolist():
            ...
            if item.filename == 'EPUB/content.opf':
                ...
                # Template v4: MOVE nav itemref ПОСЛЕ первого content itemref (preface = Prefácio)
                ...
                if nav_match:
                    ...
                    if intro_match:
                        ...
                        nav_moved = 1
                        print(f'  [OK] Template v4: nav itemref MOVED после ch001 (Prefácio) per Iskra POMETKA-S318-05 §2 п.3')
                    else:
                        ...
                        if first_chapter:
                            ...
                            if match:
                                ...
                                print(f'  [WARN] Template v4: fallback failed, nav removed')
                                content = content_no_nav
                        else:
                            print(f'  [WARN] Template v4: no chapter itemref found, nav removed')
                            content = content_no_nav
                else:
                    print(f'  [INFO] Template v4: no nav itemref in spine to move')
                data = content.encode('utf-8')
            # v1.0.1 Template v4: KEEP landmarks TOC reference (nav IS в spine, no removal)
...
# Gate 1 Template v4: nav MUST be present в spine (post-Prefácio) per Iskra POMETKA-S318-05 §2 п.3
