...
async function renderMarkdown(body: string): Promise<string> {
  ...
  // smartypants converts «---» → «—», «--» → «–», quotes к smart curly. Improves
  // typography для pandoc HTML→MD output that encodes em-dashes as «---».
  marked.use({ gfm: true })
...
async function updateRender() {
  ...
  const rawBody = await loadKn5ChapterBody(currentSlug)
  if (!rawBody) {
    isLoading.value = false
    return
  }
...
<template>
  <main class="reader-chapter">
    <nav class="reader-chapter__breadcrumb" aria-label="Хлебные крошки">
      ...
      <span>›</span>
      <RouterLink to="/kn5/read">Оглавление</RouterLink>
      <span>›</span>
    ...
    <div v-else class="reader-chapter__not-found">
      ...
      <p>
        Проверьте адрес, или
        <RouterLink to="/kn5/read">вернитесь к оглавлению</RouterLink>.
      </p>
    </div>
  </main>
</template>
