...
/**
 * kn.5 «Трудности диалога» reader single-chapter (/kn7/read/:slug).
 * VIT-KLB cont+51 mirror kn1 pattern: SSR-safe pre-rendered HTML via onServerPrefetch.
 * Marked GFM config now applied at build time в kn7-reader-manifest.mjs.
 */
...
import {
  useKn7ChapterMeta,
  loadKn7ChapterBodyHtml,
} from '../composables/useKn7Chapters'

const route = useRoute()
const slug = computed(() => route.params.slug as string)
const chapterData = computed(() => useKn7ChapterMeta(slug.value))
...
onServerPrefetch(async () => {
  const html = await loadKn7ChapterBodyHtml(slug.value)
...
if (typeof window !== 'undefined') {
  ...
  watch(
    ...
    async (newSlug) => {
      ...
      const html = await loadKn7ChapterBodyHtml(newSlug)
...
useHead({
  title: () =>
    ...
    ' — Трудности диалога — Библиотека FolkUp',
  meta: () => [
    {
      name: 'description',
      content:
        ...
        'Читать онлайн — глава книги «Трудности диалога».',
    },
    {
      property: 'og:title',
    ...
    {
      property: 'og:url',
      content: `${SITE_URL}/kn7/read/${slug.value}/`,
    },
  ],
  link: () => [
    { rel: 'canonical', href: `${SITE_URL}/kn7/read/${slug.value}/` },
  ],
})
</script>
...
<template>
  <main class="reader-chapter">
    <nav class="reader-chapter__breadcrumb" :aria-label="labels.breadcrumbAria">
      ...
      <span>›</span>
      <RouterLink to="/kn7">Трудности диалога</RouterLink>
      <span>›</span>
      <RouterLink to="/kn7/read">{{ labels.breadcrumbToc }}</RouterLink>
      <span>›</span>
    ...
    <article v-if="chapterData.meta" class="reader-chapter__body">
      ...
      <!-- v-html: контент из trusted source pre-rendered by marked at build time
           (см. scripts/kn7-reader-manifest.mjs). Sync render — SSR-safe. -->
      ...
      <nav class="reader-chapter__nav" :aria-label="labels.navChaptersAria">
        <RouterLink
          ...
          :to="`/kn7/read/${chapterData.prev.slug}`"
        ...
        <RouterLink to="/kn7/read" class="reader-chapter__nav-link reader-chapter__nav-toc">
          К оглавлению
        </RouterLink>
        <RouterLink
          v-if="chapterData.next"
          :to="`/kn7/read/${chapterData.next.slug}`"
    ...
    <div v-else class="reader-chapter__not-found">
      ...
      <p>
        Проверьте адрес, или
        <RouterLink to="/kn7/read">{{ labels.notFoundLink }}</RouterLink>.
      </p>
    </div>
  </main>
</template>
...
/* Canon Iskra S138 seal reuse — circular «ЭКСПОНАТ НА РЕСТАВРАЦИИ · 2026» stamp + метафора + hint. */
.reader-chapter__content :deep(.restoration-stub) { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.6em; padding: 2em 1em; }
.reader-chapter__content :deep(.restoration-stub__seal) { width: 180px; height: 180px; transform: rotate(-8deg); opacity: 0.85; }
.reader-chapter__content :deep(.restoration-stub__metaphor) { font-family: var(--font-body, 'Vollkorn', Georgia, serif); font-size: 1.35rem; font-style: italic; color: var(--color-text, #1a1a1a); margin-top: 0.4em; }
.reader-chapter__content :deep(.restoration-stub__hint) { font-size: 0.85rem; color: var(--color-text-muted, #888); letter-spacing: 0.05em; }
@media (max-width: 640px) {
  .reader-chapter__content :deep(.restoration-stub__seal) { width: 140px; height: 140px; }
  .reader-chapter__content :deep(.restoration-stub__metaphor) { font-size: 1.15rem; }
}
